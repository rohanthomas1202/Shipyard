...
- [ ] **INFRA-01**: All subprocess calls in executor and validator nodes are async (non-blocking), preventing event loop stalls that kill WebSocket connections
- [ ] **INFRA-02**: SQLite database runs in WAL mode for concurrent read/write access
- [ ] **INFRA-03**: All LLM calls use OpenAI structured outputs with strict: true to eliminate JSON parse failures
